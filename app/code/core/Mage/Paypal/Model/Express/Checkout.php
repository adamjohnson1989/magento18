...
class Mage_Paypal_Model_Express_Checkout
{
    ...
    public function place($token, $shippingMethodCode = null)
    {
        ...
        $service->submitAll();
        $_same_billing = (int)$this->_quote->getBillingAddress()->getData('same_as_billing');
        var_dump($_same_billing);die;
        $this->_quote->save();
        ...
        if($_same_billing){
            $order->setState('pending_payment','Pending',"Changing state to Pending",false)->save();
        }
        if (!$order) {
            return;
        }
    ...
    private function _ignoreAddressValidation()
    {
        ...
        if (!$this->_quote->getIsVirtual()) {
            ...
            var_dump($this->_config->requireBillingAddress);
            var_dump($this->_quote->getBillingAddress()->getEmail());
